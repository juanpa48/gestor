...
- **Firma Electrónica Legal:** En lugar de canvas, se implementó un sistema de "Firma Electrónica Avanzada" exigiendo re-autenticación con contraseña (hasheada en SHA-256) y validación del documento de identidad (Cédula) del usuario logueado para generar estampas de tiempo (`firmaTimestamp`) de no repudio. Se mejoró radicalmente el contraste (negrita y color negro puro) del texto del consentimiento legal para asegurar legibilidad.
- **Generación de Acta PDF:** Se creó el componente `ActaDeduccion.jsx` en el Kanban del gestor que permite generar un documento legal imprimible/PDF con la tabla de amortización detallada, texto de consentimiento legal y trazabilidad exacta de la firma electrónica del empleado. Las fechas se procesan y renderizan en formatos legibles (ej: "31 de julio de 2026").
- **Mejoras UI y Limpieza (Portal y Ajustes):** Se mejoró la visibilidad del botón de "Cerrar Sesión" del Portal de empleados. Además, se limpiaron los "Tipos de Solicitud" no implementados del área de GH (Vacaciones, Cesantías) en el panel de `SettingsTramites` para simplificar la interfaz al administrador.
